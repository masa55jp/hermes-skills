...
description: Design and draw high-quality game characters, mascots, avatars, creatures and item/portrait art as hand-written SVG. Use this whenever the user asks for a character, mascot, avatar, monster, hero, NPC, sprite, icon-style creature, or any illustrated figure in SVG form — even if they just say "draw a knight" or "make me a cute robot" without saying SVG. Also use it to improve, restyle, or fix an existing character SVG. Covers concept sheet → silhouette → layered construction → render-and-critique loop.
version: 1.0.0
...
metadata:
  hermes:
    tags: [svg, character-design, game-art, illustration, mascot, vector, concept-art]
    related_skills: [pixel-art, concept-diagrams, claude-design]
---

# SVG Character Design

Write game-ready character art as clean, layered, hand-authored SVG. This skill is tuned for
...
```
Subject:      what it is (species/role/gender presentation/age feel)
Style:        one of the presets in references/style-presets.md (default: flat-vector-cel)
Proportion:   heads-tall (chibi 2–2.5 / cute 3–4 / stylized 5–6 / heroic 7–8)
Pose:         static front / 3/4 / action; what the hands and feet are doing
Silhouette:   the ONE shape that identifies the character from a black shadow
Palette:      5–7 colors max: base, base-shadow, skin, accent, dark outline, highlight, background(optional)
Key props:    ≤3 (weapon, hat, tail, etc.)
Canvas:       viewBox size and where the feet/base line sits
```
...
Every group gets an `id`. Parts that connect (arm→torso, head→neck) must overlap by 10–20% of the
smaller part so joints never show gaps.
...
5. **Face.** This is where quality is judged. Follow `references/anatomy-and-proportions.md`.
...
For style-specific recipes (chibi, kawaii mascot, dark fantasy, sci-fi mech, cute monster, JRPG hero,
Western cartoon) load `references/style-presets.md`. For SVG syntax reminders and path tricks load
...
3. Eyes level & equal, expression clear? yes / no → which
...
Fix only what the checklist flagged, in priority order: silhouette → connection → face → lighting →
polish. Re-render after each round. Stop after three rounds or when all seven checks pass; then tell
