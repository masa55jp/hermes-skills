...
| [`comfy-rtx3060`](creative/comfy-rtx3060/SKILL.md) | Everything a general ComfyUI guide cannot know about this particular machine: where its ComfyUI is, that the GPU is exclusive so the service may simply not be running, and that these models load as diffusion units rather than checkpoints — so an empty checkpoint list is correct, not a fault. Ships a verified generator script; self-sufficient by design, because an agent will not follow a pointer to another skill. |
...
| [`creative/comfy-rtx3060`](creative/comfy-rtx3060/SKILL.md) | この環境のComfyUIがどこにあるか、4モデルの使い分け、GPUが排他で落ちている可能性、そして**モデルがチェックポイントではなく拡散モデルとして入っている**こと（＝チェックポイントが空なのは正常）。検証済みの生成スクリプト同梱。★落ちていたら自分で切り替えず人に頼む／★他スキルへの委譲はしない（エージェントは辿らない） |
